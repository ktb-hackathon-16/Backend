...
public class MessagePaginationRepositoryImpl
    ...
    public List<Message> findOlderMessages(
        ...
        if (cursor != null) {
            LocalDateTime cursorTimestamp = LocalDateTime.ofInstant(
    ...
    private void validateArguments(
        ...
        if (cursor != null && !cursor.isValid()) {
            throw new IllegalArgumentException("유효하지 않은 메시지 커서입니다.");
        }
        if (fetchSize < 1) {
            throw new IllegalArgumentException("fetchSize는 1 이상이어야 합니다.");
        }
    }
}
