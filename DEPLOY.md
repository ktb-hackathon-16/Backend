...
운영 서버에서는 repo의 `docker-compose.prod.yaml`을 배포하고, 실제 환경 변수는
서버의 `/etc/ktb/backend-app.env`에만 둡니다. 로컬 개발용 `apps/backend/.env`는
운영 배포에 사용하지 않습니다.

최초 1회, Backend EC2에서 운영 env 파일을 고정합니다.

```bash
sudo mkdir -p /etc/ktb
sudo cp /tmp/backend-app.env /etc/ktb/backend-app.env
...
서버에 `docker compose` 플러그인이 없고 `docker-compose` v1만 있으면 마지막 줄은
다음처럼 실행합니다.
...
- 이미지: `youngjin179/ktb-backend:1.0.0`
- env 파일: `/etc/ktb/backend-app.env`
- HTTP API: `8080 -> 5001`
- Socket.IO: `5002 -> 5002`

필요하면 실행 시점에만 override할 수 있습니다.

```bash
BACKEND_IMAGE=youngjin179/ktb-backend:1.0.1 \
